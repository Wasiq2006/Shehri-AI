...
export default function HeroView({ apiBase, onReport }) {
  ...
  return (
    <div className="flex-1 flex flex-col justify-between pt-4 px-5 pb-28 max-w-[430px] mx-auto min-h-screen fade-in">
      ...
      <div className="my-6">
        <div className="bg-white rounded-[28px] p-6 shadow-soft border border-slate-100/60 relative overflow-hidden flex flex-col items-center text-center">
          ...
          <div className="mt-4 flex items-center space-x-3 w-full">
            <button className="flex-1 py-3 px-4 rounded-full bg-stone-50 hover:bg-stone-100 active:scale-[0.98] text-slate-700 font-medium text-xs border border-slate-200/70 transition-all flex items-center justify-center space-x-1.5 disabled:opacity-50" disabled={loading}>
      ...
      {/* Bottom Civic Metrics Card */}
      <div className="bg-white rounded-[24px] p-4 shadow-soft border border-slate-100/60 mb-2">
        <div className="flex items-center justify-between text-xs px-2">
          <div className="flex items-center space-x-2.5">
            <div className="w-7 h-7 rounded-full bg-emerald-50 text-emerald-800 flex items-center justify-center">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4"><circle cx="12" cy="12" r="10"></circle><path d="m16 9-5.5 5.5L8 12"></path></svg>
            </div>
            <div>
              <div className="font-semibold text-slate-900">{totalCases} Cases Resolved</div>
              <div className="text-[11px] text-slate-400">Past 30 days in your area</div>
            </div>
          </div>
          <span className="text-[11px] font-semibold text-emerald-800">96.4% SLA</span>
        </div>
      </div>
